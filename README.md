...
│   ├── test-admin-ui.js 菜单管理 / 业绩报表页 DOM 88 项（§32 权限分界）
│   └── harness.js       测试框架（零依赖）
│
├── .github/workflows/
...
npm run test:admin-ui    # 菜单管理 / 业绩报表页真的开起来跑（88 项，含 §32 权限分界）
...
> 之后改后端只要 `git push`：CI 会先跑 2370 项测试，再用 `clasp` 部署，
...
| 测试 | `test:upgrade` 142 · `test:menu` 139 · `test:menuui` 88 · `test:checkout` 155 · `test:orderboard` 159 · `test:orderui` 119 · `test:security` 214 · `test:analytics` 103 · `test:mvp` 115 · `test:admin-ui` 88 |
