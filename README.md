...
>   `js/config.js`、`service-worker.js` 快取名全部 = **2.1.10**
...
>   `✓ 后端 v2.1.10 · 已是最新版`，若还贴着旧版会红字提示重新贴 Apps Script
> · 规则：**任何改动都先更新 GitHub 上这个档案**，再从 GitHub 贴到 Apps Script
> · 2.1.10：新订单会**念出「您有新订单」**（浏览器内建语音，免下载）
...
>   失败会自动退回原来那一栏。
> · 从 2.1.8 起，所有页面的 css / js 都带版本号（`admin.css?v=2.1.10`）：
...
- `✓ 后端 v2.1.10 · 已是最新版` → 贴对了
- `⚠ 后端 vX ≠ 前端 v2.1.10 · 请重新贴 Apps Script` → 还是旧版
