/* =============================================================
   YETIPSY MINI APP 2.1.9 — Code.gs（唯一的后端档案）
   -------------------------------------------------------------
   ...
   怎么贴（只有这一个档案，不用再贴 20 个）：
     ...
     5. 回员工端「MORE」页最下面看：✓ 后端 v2.1.9 · 已是最新版

   版本 2.1.9 · 改完跑 npm run check:backend（会用这份档案跑一次端到端）
...
/* ===== [1/20] Config.gs — 所有设定 + 17 张表定义（要改规则改这一段） ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.9 — Config.gs
   -------------------------------------------------------------
...
/* ===== [2/20] Utils.gs — 公用工具：电话 E.164、错误码、日期、金额、回应格式 ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.9 — Utils.gs
   -------------------------------------------------------------
...
/* ===== [3/20] Database.gs — Sheets 存取层：setupDatabase / upgradeToV2 / bootstrapOwner / dedupeCustomers ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.9 — Database.gs
   -------------------------------------------------------------
   Google Sheets 存取层。
...
/* ===== [4/20] Security.gs — Session token、角色权限、Rate limit、登入锁定 ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.9 — Security.gs
   -------------------------------------------------------------
...
/* ===== [5/20] Audit.gs — Audit Log ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.9 — Audit.gs
...
/* ===== [6/20] Points.gs — 积分累计 / 等级门槛 ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.9 — Points.gs
...
/* ===== [7/20] Rewards.gs — 奖励产生与状态 ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.9 — Rewards.gs
...
/* ===== [8/20] Wallet.gs — 钱包储值 / 抵扣 / 上限 ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.9 — Wallet.gs
...
/* ===== [9/20] Customers.gs — 会员：查号码 / 注册 / 密码登录 / 改密码 / 会员条码 ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.9 — Customers.gs
   -------------------------------------------------------------
   会员注册 / 登录 / 资料
...
/* ===== [10/20] Orders.gs — 消费纪录与统计 ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.9 — Orders.gs
   -------------------------------------------------------------
   「已验证会员消费记录」= 积分的唯一来源。
...
/* ===== [11/20] Menu.gs — 酒单：分类 / 商品 / 规格 / 售罄 / 菜单快取 ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.9 — Menu.gs（2.0 Phase 3）
   -------------------------------------------------------------
   酒单：分类 / 商品 / 规格（§5–§8）
...
/* ===== [12/20] Checkout.gs — 结帐报价：后端重算价格、钱包上限、Quote 5 分钟、防重复下单 ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.9 — Checkout.gs（2.0 Phase 5）
   -------------------------------------------------------------
   结帐报价（§42 / §43 / §44）
...
/* ===== [13/20] AppOrders.gs — 订单：placeOrder（幂等）、查询、取消、再点一次 ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.9 — AppOrders.gs（2.0 Phase 6）
   -------------------------------------------------------------
...
/* ===== [14/20] OrderBoard.gs — 员工看板：接单 / 制作 / 完成（幂等）、收款才扣钱包、6 小时到店判定 ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.9 — OrderBoard.gs（2.0 Phase 7）
   -------------------------------------------------------------
...
/* ===== [15/20] Analytics.gs — 业绩：今日统计、通路业绩、热销商品、会员分析 ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.9 — Analytics.gs（2.0 Phase 11）
   -------------------------------------------------------------
   业绩分析（§50 §51 §52 §62）
...
/* ===== [16/20] Claims.gs — Claim QR / Code、认领；2.1 POS 进单（createPosTicket / getPosQueue / bindPosTicket / cancelPosTicket） ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.9 — Claims.gs
   -------------------------------------------------------------
...
function createPosTicket(data, token) {
  ...
  /* 2.1.9 结账：付款方式也写进 Note（AUDIT 有纪录；Orders 表不加栏位） */
...
/* ===== [17/20] Promotions.gs — 优惠规则 ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.9 — Promotions.gs
   ============================================================= */
...
/* ===== [18/20] Admin.gs — 员工端：Dashboard、会员查询、手动调整、重设密码、设置 ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.9 — Admin.gs
...
/* ===== [19/20] Auth.gs — ping / getPublicSettings / staffLogin / staffLogout ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.9 — Auth.gs
   -------------------------------------------------------------
...
/* ===== [20/20] Code.gs — 唯一入口 doPost() / doGet()：action 白名单、参数解析、交易锁、错误包装 ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.9 — Code.gs
   -------------------------------------------------------------
   Web App 入口。
