...
/* ===== [1/20] Config.gs — 所有设定 + 17 张表定义（要改规则改这一段） ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.14 — Config.gs
   -------------------------------------------------------------
...
function defaultSettings() {
  return {
    ...
    ALLOW_POS_MANUAL_VERIFY:       'TRUE',   // POS：顾客没带会员码时，员工可用手机号搜寻会员后确认进分（会写 AUDIT）
...
var SETTING_DESC = {
  ...
  ALLOW_POS_MANUAL_VERIFY:       'POS: allow manual member verify (no member code) / POS：允许用手机号搜寻会员后确认进分',
...
/* ===== [2/20] Utils.gs — 公用工具：电话 E.164、错误码、日期、金额、回应格式 ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.14 — Utils.gs
   -------------------------------------------------------------
...
/* ===== [3/20] Database.gs — Sheets 存取层：setupDatabase / upgradeToV2 / bootstrapOwner / dedupeCustomers ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.14 — Database.gs
   -------------------------------------------------------------
   Google Sheets 存取层。
...
/* ===== [4/20] Security.gs — Session token、角色权限、Rate limit、登入锁定 ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.14 — Security.gs
   -------------------------------------------------------------
...
/* ===== [5/20] Audit.gs — Audit Log ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.14 — Audit.gs
...
/* ===== [6/20] Points.gs — 积分累计 / 等级门槛 ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.14 — Points.gs
...
/* ===== [7/20] Rewards.gs — 奖励产生与状态 ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.14 — Rewards.gs
...
/* ===== [8/20] Wallet.gs — 钱包储值 / 抵扣 / 上限 ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.14 — Wallet.gs
...
/* ===== [9/20] Customers.gs — 会员：查号码 / 注册 / 密码登录 / 改密码 / 会员条码 ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.14 — Customers.gs
   -------------------------------------------------------------
   会员注册 / 登录 / 资料
...
/* ===== [10/20] Orders.gs — 消费纪录与统计 ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.14 — Orders.gs
   -------------------------------------------------------------
   「已验证会员消费记录」= 积分的唯一来源。
...
/* ===== [11/20] Menu.gs — 酒单：分类 / 商品 / 规格 / 售罄 / 菜单快取 ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.14 — Menu.gs（2.0 Phase 3）
   -------------------------------------------------------------
   酒单：分类 / 商品 / 规格（§5–§8）
 ...
 * 2.1.11 · 「上架」判定
 * -------------------------------------------------------------
 * 员工端菜单管理页一直把「Status 空白」当成上架（可以直接按「下架」），
 * 但这里以前要求 Status 必须写 ACTIVE → 直接从 Sheet 新增、
 * 或 1.x 年代留下来的商品全都不会出现在点餐台（员工以为上架了，
 * 点餐台却只有一两个商品）。两边现在用同一个判断。
 */
function isMenuLive(status) {
...
/* ===== [12/20] Checkout.gs — 结帐报价：后端重算价格、钱包上限、Quote 5 分钟、防重复下单 ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.14 — Checkout.gs（2.0 Phase 5）
   -------------------------------------------------------------
   结帐报价（§42 / §43 / §44）
...
/* ===== [13/20] AppOrders.gs — 订单：placeOrder（幂等）、查询、取消、再点一次 ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.14 — AppOrders.gs（2.0 Phase 6）
   -------------------------------------------------------------
...
/* ===== [14/20] OrderBoard.gs — 员工看板：接单 / 制作 / 完成（幂等）、收款才扣钱包、6 小时到店判定 ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.14 — OrderBoard.gs（2.0 Phase 7）
   -------------------------------------------------------------
 ...
 * 2.1.11 · 看板快照
 * -------------------------------------------------------------
 * 看板要的 4 个栏位 + 今日统计 + 接单开关 + 轮询秒数，一次算完。
 * getActiveOrders 用它；状态推进的 action（接单 / 制作 / 好了 / 完成 /
 * 收款 / 取消）也把最新快照一起回传 —— 这样员工按一下，画面直接
 * 用回传的资料更新，不用再等第二个请求（Apps Script 一个来回常常
 * 1~3 秒，现场感觉就是「点了会延迟」）。
 */
...
/** 旧版内联写法（保留给需要看历史的读者；2.1.11 起改用 boardSnapshot） */
function getActiveOrdersLegacy(data, token) {
  var ctx = requireStaff(token);
  if (ctx.error) return ctx.error;
...
function acceptOrder(data, token) {
  ...
  var wantStart = !!(data && (data.startPreparing === true ||
                              data.startPreparing === 'TRUE' || data.startPreparing === 1));

  /* 2.1.11 幂等：员工重按、或另一位员工先按了 → 不要报错，
     直接回最新状态（现场最讨厌「按了说我做错了」）。 */
  if (o.orderStatus === 'PREPARING' ||
      (o.orderStatus === 'CONFIRMED' && !wantStart)) {
    return ok({ order: boardOrder(o), idempotent: true, snapshot: boardSnapshot() });
  }
  if (o.orderStatus === 'CONFIRMED' && wantStart) {
    o.orderStatus = 'PREPARING';
    o.startedAt = nowISO();
    o.updatedAt = nowISO();
    audit(staff.staffId, 'STAFF', 'START_PREPARING', 'APP_ORDER', o.appOrderId,
          'CONFIRMED', 'PREPARING');
    return ok({ order: boardOrder(o), startPreparing: true, idempotent: true, snapshot: boardSnapshot() });
  }
  ...
  /* 2.1.11 员工要的流程是「按一下就进制作」：
     data.startPreparing = true 时，同一个请求直接接到 PREPARING。
     省掉第二个来回，现场按下去就是「制作中」。 */
  if (wantStart) {
    o.orderStatus = 'PREPARING';
    o.startedAt = nowISO();
    ...
    audit(staff.staffId, 'STAFF', 'START_PREPARING', 'APP_ORDER', o.appOrderId,
          'CONFIRMED', 'PREPARING');
    return ok({ order: boardOrder(o), startPreparing: true, snapshot: boardSnapshot() });
  }

  return ok({ order: boardOrder(o), snapshot: boardSnapshot() });
}
...
function startPreparing(data, token) {
  ...
  /* 2.1.11 幂等：已经制作中 / 已经做好 → 重复点不要报错 */
  if (o.orderStatus === 'PREPARING' || o.orderStatus === 'READY') {
    return ok({ order: boardOrder(o), idempotent: true, snapshot: boardSnapshot() });
  }
...
function markReady(data, token) {
  ...
  /* 2.1.11 幂等：已经可取酒 / 已完成 → 重复点不要报错 */
  if (o.orderStatus === 'READY' || o.orderStatus === 'COMPLETED') {
    return ok({ order: boardOrder(o), idempotent: true, snapshot: boardSnapshot() });
  }
...
function markPaymentPaid(data, token) {
  ...
  if (o.paymentStatus === 'PAID') {
    return ok({ order: boardOrder(o), alreadyPaid: true, snapshot: boardSnapshot() });
  }
  ...
  return ok({ order: boardOrder(o), walletUsed: used, snapshot: boardSnapshot() });
}
...
function cancelAppOrder(data, token) {
  ...
  if (o.orderStatus === 'CANCELLED') {
    return ok({ order: boardOrder(o), alreadyCancelled: true, refunded: 0, snapshot: boardSnapshot() });
  }
  if (o.orderStatus === 'COMPLETED') {
  ...
  return ok({ order: boardOrder(o), refunded: refunded, snapshot: boardSnapshot() });
}
...
/* ===== [15/20] Analytics.gs — 业绩：今日统计、通路业绩、热销商品、会员分析 ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.14 — Analytics.gs（2.0 Phase 11）
   -------------------------------------------------------------
   业绩分析（§50 §51 §52 §62）
...
/* ===== [16/20] Claims.gs — Claim QR / Code、认领；2.1 POS 进单（createPosTicket / getPosQueue / bindPosTicket / cancelPosTicket） ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.14 — Claims.gs
   -------------------------------------------------------------
...
/** POS 结账可选的付款方式（前端只送这几个，其余忽略） */
var POS_PAYMENT_METHODS = ['CASH', 'CARD', 'DUITNOW', 'COUNTER', 'PAID', 'FREE'];
...
function createPosTicket(data, token) {
  ...
  /* 2.1.11 结账：付款方式也写进 Note（AUDIT 有纪录；Orders 表不加栏位） */
  var pay = String((data && data.paymentMethod) || '').toUpperCase();
  if (POS_PAYMENT_METHODS.indexOf(pay) === -1) pay = '';

  var note = summary ? (staffNote ? summary + ' · ' + staffNote : summary) : staffNote;
  if (note && pay) note = note + ' · ' + pay;
  else if (!note && pay) note = pay;
  ...
  audit(ctx.staff.staffId, 'STAFF', 'POS_TICKET', 'ORDER', order.orderId, '',
        amount + (pay ? ' | ' + pay : '') + (items.length ? ' | ' + items.length + ' items' : ''));
  var view = posTicketView(order);
  view.paymentMethod = pay;
  view.items = items;
  return ok({ ticket: view, items: items });
}
...
/**
 * posVerifyMember —— POS 结账用：顾客没有会员码（手机没电 / 没带）时，
 * 员工用手机号或会员编号找到会员、当面核对姓名之后取得 verifyToken，
 * 后续 bindPosTicket 就能进分。
 *
 * 会在 AUDIT 留下 'POS_VERIFY_MANUAL' 纪录；若老板不要这条路径，
 * 把 Settings 的 ALLOW_POS_MANUAL_VERIFY 改成 FALSE 就会关掉。
 */
function posVerifyMember(data, token) {
  var ctx = requireStaff(token);
  if (ctx.error) return ctx.error;
  if (!boolSetting('ALLOW_POS_MANUAL_VERIFY', true)) {
    return err('MANUAL_VERIFY_DISABLED',
      'Manual member verify is off. Please ask the customer for their member code. / ' +
      '手动验证已关闭，请顾客出示会员码。');
  }

  var customerId = String((data && data.customerId) || '').trim();
  if (!customerId) return err('CUSTOMER_NOT_FOUND');

  var c = dbById('customers', customerId);
  if (!c) return err('CUSTOMER_NOT_FOUND');
  if (String(c.status || 'ACTIVE').toUpperCase() === 'BLOCKED') return err('CUSTOMER_BLOCKED');
  ...
  audit(ctx.staff.staffId, 'STAFF', 'POS_VERIFY_MANUAL', 'CUSTOMER', c.customerId, '',
        maskName(c.name));
...
/* ===== [17/20] Promotions.gs — 优惠规则 ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.14 — Promotions.gs
   ============================================================= */
...
/* ===== [18/20] Admin.gs — 员工端：Dashboard、会员查询、手动调整、重设密码、设置 ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.14 — Admin.gs
...
/* ===== [19/20] Auth.gs — ping / getPublicSettings / staffLogin / staffLogout ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.14 — Auth.gs
   -------------------------------------------------------------
...
/* ===== [20/20] Code.gs — 唯一入口 doPost() / doGet()：action 白名单、参数解析、交易锁、错误包装 ===== */
/* =============================================================
   YETIPSY MINI APP 2.1.14 — Code.gs
   -------------------------------------------------------------
   Web App 入口。
...
function getHandlers() {
  return {
    ...
    posVerifyMember: posVerifyMember,
